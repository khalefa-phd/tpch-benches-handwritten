...
void filter_and_hash_customers(Customer *c, int partition,
...
void join_with_orders(Order *o, int partition, unordered_set<int> *customers,
...
void join_with_lineitems(Lineitem *l, int partition,
                         unordered_map<int, HashEntry *> *orders_map) {
  int cutoff_date = 19950324;
...
void with_sync(Customer *customers, Order *orders, Lineitem *lineitems) {
  ...
  unordered_map<int, HashEntry *> *orders_map =
      new unordered_map<int, HashEntry *>();
...
#pragma omp parallel for
  ...
  for (result_it = orders_map->begin(); result_it != orders_map->end();
       result_it++) {
...
void run_partition(Customer *c, Order *o, Lineitem *l, int partition,
                   unordered_map<int, double> *orders_map) {
  int cutoff_date = 19950324;
  int start = (partition * num_lineitems) / NUM_PARALLEL_THREADS,
      end = ((partition + 1) * num_lineitems) / NUM_PARALLEL_THREADS;
  int order_index =
      binary_search(o->orderkey, ORDERS_PER_SF * SF, l->orderkey[start]);
  for (int i = start; i < end; i++) {
    if (l->shipdate[i] > cutoff_date) {
      int orderkey = l->orderkey[i];
      while (o->orderkey[order_index] != orderkey)
        order_index++;
...
#pragma omp critical(mapupdate)
          {
            (*orders_map)[orderkey] +=
                l->extendedprice[i] * (1 - l->discount[i]);
          }
        }
      }
    }
...
void run_partition_nosync(Customer *c, Order *o, Lineitem *l, int partition,
                          double *result) {
  int cutoff_date = 19950324;
  int start = (partition * ORDERS_PER_SF * SF) / NUM_PARALLEL_THREADS,
      end = ((partition + 1) * ORDERS_PER_SF * SF) / NUM_PARALLEL_THREADS;
      ...
      if (c->mktsegment[custkey] == 1) {
        ...
        while (l->orderkey[li_index] == orderkey) {
          if (l->shipdate[li_index] > cutoff_date) {
            result[i] +=
                l->extendedprice[li_index] * (1 - l->discount[li_index]);
          }
          li_index++;
        }
      }
...
void run_partition_nosync_joined(Customer *c, Order *o, Lineitem *l,
                                 int partition, double *result) {
  int cutoff_date = 19950324;
  int start = (partition * ORDERS_PER_SF * SF) / NUM_PARALLEL_THREADS,
      end = ((partition + 1) * ORDERS_PER_SF * SF) / NUM_PARALLEL_THREADS;
  // cout << "here " << partition << "\t" << start << "\t" << end << "\t"
  //   << "\n";
  for (int i = start; i < end; i++) {
    if (o->orderdate[i] < cutoff_date) {
      int custkey = o->custkey[i] - 1;
      if (c->mktsegment[custkey] == 1) {
        int orderkey = o->orderkey[i];
        for (int li_index = o->li_start[i]; li_index < o->li_end[i];
             li_index++) {
          if (l->shipdate[li_index] > cutoff_date) {
            result[i] +=
                l->extendedprice[li_index] * (1 - l->discount[li_index]);
...
void assuming_sorted_nosync(Customer *customers, Order *orders,
                            Lineitem *lineitems) {
  struct timeval before, after, diff;
  gettimeofday(&before, 0);
...
void complete_query_joined(Customer *customers, Order *orders,
                           Lineitem *lineitems, int id, Result *results) {
  struct timeval before, after, diff;
...
#pragma omp parallel for
  ...
  int count = 0;
  // TODO: Maybe use vector here ?
  results = new Result[ORDERS_PER_SF * SF];
  for (int i = 0; i < ORDERS_PER_SF * SF; i++) {
    if (result[i] != 0.0) {
      Result &res = results[count];
      res.orderkey = orders->orderkey[i];
  ...
  */
  printf("Run %d \n \tResult cardinality: %d\n", id, count);
  printf("\t Q3 Complete: %ld.%06ld  (s)   %ld(microsecond) \n",
         (long)diff.tv_sec, (long)diff.tv_usec,
         ((long)diff.tv_sec * 1000 * 1000) + (long)diff.tv_usec);
  delete result;
}

void loadData_q3(string data_dir, Customer *customers, Order *orders,
                 Lineitem *lineitems) {
...
int main(int argc, char **argv) {
  ...
  if (!load_sf(argc, argv, SF, dir)) {
    printf("Run as ./q3 -dir <dir> -sf <sf>\n");
    return 0;
  }
  ...
  for (int i = 0; i < ORDERS_PER_SF * SF; i++) {
    ...
    while (lineitems->orderkey[li_index] != orderkey)
      li_index++;
  ...
  for (int i = 0; i < 10; i++) {
    Result *rs;
    complete_query_joined(customers, orders, lineitems, i, rs);
    escape(rs);
  }
