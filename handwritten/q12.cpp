...
void partition_withsync(Order *o, Lineitem *l, int partition,
                        int results[2][2]) {
...
#pragma omp critical(merge)
  ...
  printf("Parititon %d -  %ld.%06ld\n", partition, (long)diff.tv_sec,
         (long)diff.tv_usec);
}

void partition_nosync(Order *o, Lineitem *l, int partition, int results[2][2]) {
  int start = (partition * num_lineitems) / NUM_PARALLEL_THREADS;
  int end = ((partition + 1) * num_lineitems) / NUM_PARALLEL_THREADS;
  int order_index =
  ...
  for (int i = start; i < end; i++) {
    if (l->commitdate[i] >= l->recieptdate[i])
      continue;
    if (!(l->recieptdate[i] >= 19940101 and l->recieptdate[i] < 19950101))
      continue;
    if (l->shipdate[i] >= l->commitdate[i])
      continue;
...
int main(int argc, char **argv) {
  ...
  if (!load_sf(argc, argv, SF, dir)) {
    printf("Run as ./q12 -dir <dir> -sf <sf>\n");
    return 0;
  }
  ...
  for (int i = 0; i < num_lineitems; i++) {
    ...
    while (orders->orderkey[order_index] != orderkey)
      order_index++;
