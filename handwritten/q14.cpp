...
struct q_result execute_query(Part *p, Lineitem *l, Dict<int, long> &pk_index,
                              int tid);

double run_parallel(Part *p, Lineitem *l) {
  Dict<int, long> &pk_index =
      build_index(p->partkey, p->promo_str, SF * PARTS_PER_SF);
...
int main(int argc, char **argv) {
  ...
  if (!load_sf(argc, argv, SF, dir)) {
    printf("Run as ./q14 -dir <dir> -sf <sf>\n");
    return 0;
  }
  ...
  for (int i = 0; i < 10; i++) {
    printf("%d | %d | %d | %d |\n", parts->partkey[i], parts->brand[i],
           parts->promo_str[i], parts->size[i], parts->container[i]);
  }
