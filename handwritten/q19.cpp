...
double execute_query(Part *p, Lineitem *l, Dict<int, long> &pk_index, int tid);
...
double execute_query(Part *p, Lineitem *l, Dict<int, long> &pk_index, int tid) {
  ...
  for (int i = start; i < end; i++) {
    ...
    if ((p_brand == 12 &&
         (p_container == 11 || p_container == 16 || p_container == 17 ||
          p_container == 13) &&
         (l_quantity >= 1 && l_quantity <= 11) &&
         (p_size >= 1 && p_size <= 5) && l_shipinstruct == 1 &&
         (l_shipmode == 2 || l_shipmode == 3)) ||
        (p_brand == 23 &&
         (p_container == 24 || p_container == 26 || p_container == 23 ||
          p_container == 27) &&
         (l_quantity >= 10 && l_quantity <= 20) &&
         (p_size >= 1 && p_size <= 10) && l_shipinstruct == 1 &&
         (l_shipmode == 2 || l_shipmode == 3)) ||
        (p_brand == 34 &&
         (p_container == 31 || p_container == 36 || p_container == 37 ||
          p_container == 33) &&
         (l_quantity >= 20 && l_quantity <= 30) &&
         (p_size >= 1 && p_size <= 15) && l_shipinstruct == 1 &&
         (l_shipmode == 2 || l_shipmode == 3))) {
...
int main(int argc, char **argv) {
  ...
  printf("orderkey | quantity | extendedprice | discount | shipinstruct | "
         "shipmode | \n");
  for (int i = 0; i < 10; i++) {
    printf("%d | %d | %lf | %lf | %d | %d |\n", lineitems->orderkey[i],
           lineitems->quantity[i], lineitems->extendedprice[i],
           lineitems->discount[i], lineitems->shipinstruct[i],
  ...
  for (int i = 0; i < 10; i++) {
    printf("%d | %d | %d | %d |\n", parts->partkey[i], parts->brand[i],
           parts->size[i], parts->container[i]);
  }
